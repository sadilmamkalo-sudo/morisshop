  ...
  return (
    <>
      <button onClick={toggleCompare}
        className={`flex items-center gap-1.5 text-xs font-semibold px-4 py-3 rounded transition-all duration-300 ${isCompared ? 'bg-clay-500 text-white shadow-sm' : 'bg-ink-50 dark:bg-ink-700 text-ink-500 dark:text-ink-400 hover:bg-ink-100 dark:hover:bg-ink-600'}`}>
          ...
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-ink-800 border-t border-ink-200 dark:border-ink-700 shadow-md px-4 py-3">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <button onClick={clearAll} className="text-xs text-red-500 hover:underline px-3 py-2">Clear all</button>
              </div>
              <Link to={`/compare?ids=${compared.join(',')}`} className="btn-primary text-sm py-3 px-6">
