...
export default function ProductDetail() {
  ...
  if (!product) return <div className="flex items-center justify-center h-96"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-moris-500"></div></div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-moris-50 via-yellow-50 to-orange-50 dark:from-dark-950 dark:via-dark-900 dark:to-dark-950 py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-2 gap-12">
            ...
            <div className="relative glass-card overflow-hidden p-2">
              <div className="h-96 rounded-2xl overflow-hidden">
                {product.images?.[activeImg] ? (
                  ...
                  <div className="flex items-center justify-center h-full"><svg className="w-24 h-24 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>
                )}
              </div>
              {discount > 0 && (
                <span className="absolute top-6 left-6 bg-gradient-to-r from-red-500 to-orange-500 text-white text-sm px-4 py-1.5 rounded-full font-bold shadow-lg">-{discount}%</span>
              )}
            </div>
            {product.images?.length > 1 && (
              <div className="flex gap-3">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImg(i)} className={`w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all duration-300 hover:shadow-lg ${i === activeImg ? 'border-moris-500 shadow-lg shadow-moris-500/20' : 'border-white/20 opacity-70 hover:opacity-100'}`}>
          ...
          <div>
            <h1 className="text-4xl font-bold font-display text-gradient mb-4">{name}</h1>
            <div className="flex items-center gap-2 mb-4">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className={`w-5 h-5 ${i < Math.round(product.rating || product.averageRating) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
              ))}
              <span className="text-gray-500">({product.numReviews} reviews)</span>
            </div>

            <div className="flex items-end gap-4 mb-6">
              <span className="text-4xl font-bold text-moris-500">{product.price} <span className="text-lg">DH</span></span>
              {product.oldPrice > product.price && (
                <span className="text-xl text-gray-400 line-through mb-1">{product.oldPrice} DH</span>
              )}
            </div>
            ...
            <div className="space-y-4 mb-4">
              <label className="flex items-center gap-3 p-4 rounded-2xl bg-white/50 dark:bg-dark-800/50 border border-gray-200 dark:border-dark-700 cursor-pointer hover:border-moris-300 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">+25 DH — Premium gift wrap with personalized message</p>
                </div>
              </label>
              <div>
                <label className="block text-sm font-medium mb-2">Delivery Date</label>
                <input type="date" value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} min={new Date(Date.now() + 86400000).toISOString().split('T')[0]} className="input-field" />
              </div>
            </div>
            ...
            <div className="flex gap-4 mb-8">
              ...
              <button onClick={handleWishlist} className="w-14 h-14 rounded-2xl bg-white dark:bg-dark-800 border-2 border-gray-200 dark:border-gray-700 hover:border-red-300 flex items-center justify-center transition-all duration-300 hover:shadow-lg group">
            ...
            {product.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {product.tags.map((tag, i) => (
                  <span key={i} className="px-3 py-1 rounded-full bg-white/50 dark:bg-dark-800/50 text-sm text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700">{tag}</span>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="mt-16">
          <div className="card-solid p-6 mb-8">
            <h3 className="section-title mb-6">Questions & Answers</h3>
            {token ? (
              <form onSubmit={async (e) => { e.preventDefault(); if (!qaQuestion.trim()) return; try { await axios.post(`/api/products/${product._id}/question`, { question: qaQuestion }, { headers: { Authorization: `Bearer ${token}` } }); toast.success('Question submitted'); setQaQuestion(''); } catch (err) { toast.error(err.response?.data?.message || 'Error'); } }} className="flex gap-2 mb-6">
                <input type="text" value={qaQuestion} onChange={e => setQaQuestion(e.target.value)} placeholder="Ask a question about this product..." className="input-field flex-1" />
                <button type="submit" className="btn-primary">Ask</button>
              </form>
            ) : <p className="text-sm text-gray-500 mb-6">Please <Link to="/login" className="text-moris-500 hover:underline">login</Link> to ask a question.</p>}
            <div className="space-y-4">
              {qa.length === 0 && <p className="text-center text-gray-400 py-4">No questions yet. Be the first to ask!</p>}
              {qa.map((q, i) => (
                <div key={i} className="border-b border-gray-200 dark:border-dark-700 pb-4 last:border-0">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-lg bg-moris-100 dark:bg-moris-900/30 flex items-center justify-center text-moris-600 text-sm font-bold shrink-0">Q</div>
                    <div>
                      <p className="text-sm font-medium">{q.question}</p>
                      <p className="text-xs text-gray-400 mt-1">{new Date(q.createdAt).toLocaleDateString()}</p>
                      {q.answer && (
                        <div className="mt-2 p-3 bg-gray-50 dark:bg-dark-700/50 rounded-xl">
