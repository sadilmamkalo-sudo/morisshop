...
export default function AdminSettings() {
  ...
  if (loading) return <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-moris-500 border-t-transparent rounded-full animate-spin"></div></div>;
        ...
        <div className="card-solid p-8 space-y-6">
          {fields.map(f => (
            <div key={f.key}>
              ...
              <input type={f.type} value={settings[f.key] || ''} onChange={e => handleChange(f.key, e.target.value)} className="input-field w-full" />
            </div>
          ))}
