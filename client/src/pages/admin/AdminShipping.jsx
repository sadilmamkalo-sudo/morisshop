...
export default function AdminShipping() {
 ...
 <div><label className="block text-sm font-medium mb-1">City</label><input value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} className="input" required /></div>
 <div><label className="block text-sm font-medium mb-1">Region</label><input value={form.region} onChange={e => setForm({ ...form, region: e.target.value })} className="input" /></div>
 <div><label className="block text-sm font-medium mb-1">Rate (DH)</label><input type="number" value={form.rate} onChange={e => setForm({ ...form, rate: e.target.value })} className="input" required /></div>
 <div><label className="block text-sm font-medium mb-1">Free Shipping Min</label><input type="number" value={form.freeShippingMin} onChange={e => setForm({ ...form, freeShippingMin: e.target.value })} className="input" /></div>
 <div><label className="block text-sm font-medium mb-1">Est. Days</label><input type="number" value={form.estimatedDays} onChange={e => setForm({ ...form, estimatedDays: e.target.value })} className="input" /></div>
 </div>
 ...
 <tbody>
 {rates.map(r => (
 <tr key={r._id} className="border-t border-ink-100 dark:border-ink-700/50 hover:bg-ink-50 dark:hover:bg-ink-800/30">
 <td className="px-4 py-3 font-medium">{r.city}</td>
 <td className="px-4 py-3 text-sm text-ink-500">{r.region || '-'}</td>
 <td className="px-4 py-3 font-semibold text-clay-500">{r.rate} DH</td>
 ...
 <button onClick={() => handleEdit(r)} className="text-blue-500 hover:underline text-sm"> Edit</button>
 <button onClick={() => handleDelete(r._id)} className="text-red-500 hover:underline text-sm"> Delete</button>
 </div>
 </td>
 </tr>
 ))}
 {rates.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-ink-500">No shipping rates</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
