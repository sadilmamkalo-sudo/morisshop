    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card-solid p-6 mb-8">
          <div className="grid md:grid-cols-3 gap-4 mb-4">
            <div><label className="block text-sm font-medium mb-1">{t('coupon.code')}</label><input value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} className="input-field uppercase font-bold tracking-wider" required /></div>
            <div><label className="block text-sm font-medium mb-1">{t('coupon.type')}</label><select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="input-field"><option value="percentage">{t('coupon.percentage')}</option><option value="fixed">{t('coupon.fixed')}</option></select></div>
            <div><label className="block text-sm font-medium mb-1">{t('coupon.value')}</label><input type="number" value={form.value} onChange={e => setForm({ ...form, value: e.target.value })} className="input-field" required /></div>
            <div><label className="block text-sm font-medium mb-1">{t('coupon.minOrder')}</label><input type="number" value={form.minOrder} onChange={e => setForm({ ...form, minOrder: e.target.value })} className="input-field" /></div>
            <div><label className="block text-sm font-medium mb-1">{t('coupon.maxUses')}</label><input type="number" value={form.maxUses} onChange={e => setForm({ ...form, maxUses: e.target.value })} className="input-field" /></div>
            <div><label className="block text-sm font-medium mb-1">{t('coupon.expiresAt')}</label><input type="date" value={form.expiresAt} onChange={e => setForm({ ...form, expiresAt: e.target.value })} className="input-field" required /></div>
          </div>
      ...
      <div className="card-solid overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {coupons.map(c => (
              <tr key={c._id} className="border-t border-gray-100 dark:border-dark-700/50 hover:bg-gray-50 dark:hover:bg-dark-800/30 transition-colors">
                <td className="px-4 py-3">
                  <span className="font-bold font-mono tracking-wider bg-moris-50 dark:bg-moris-900/20 px-2.5 py-1 rounded-lg text-moris-600 dark:text-moris-400">{c.code}</span>
                </td>
                ...
                <td className="px-4 py-3 text-sm text-gray-500">{new Date(c.expiresAt).toLocaleDateString('fr-FR')}</td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => handleDelete(c._id)} className="text-red-500 hover:underline text-sm font-medium">🗑️ {t('common.delete')}</button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {coupons.length === 0 && <p className="text-center py-8 text-gray-500">{t('coupon.noCoupons')}</p>}
      </div>
    </div>
  );
}
