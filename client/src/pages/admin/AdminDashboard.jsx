...
export default function AdminDashboard() {
 ...
 const cards = [
 { label: t('admin.totalUsers'), value: stats?.totalUsers || 0, icon: '', color: 'bg-blue-500', link: '/admin/users' },
 { label: t('admin.totalProducts'), value: stats?.totalProducts || 0, icon: '', color: 'bg-green-500', link: '/admin/products' },
 { label: t('admin.totalOrders'), value: stats?.totalOrders || 0, icon: '', color: 'bg-purple-500', link: '/admin/orders' },
 { label: t('admin.totalRevenue'), value: `${(stats?.totalRevenue || 0).toFixed(2)} DH`, icon: '', color: 'bg-clay-500', link: '/admin/orders' },
 ];
 const menuItems = [
 ...
 { label: 'Settings', icon: '', link: '/admin/settings' },
 ];
 ...
 <Link to={card.link} className="card block p-6 group transition-all duration-300">
 <div className={`w-12 h-12 bg-clay-500 rounded flex items-center justify-center text-2xl mb-4 shadow-sm group-hover:scale-110 transition-transform`}>{card.icon}</div>
 ...
 <Link to={item.link} className="card block p-6 text-center transition-all duration-300 group">
 ...
 {pieData.length > 0 && (
 <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card p-6">
 <h2 className="text-xl font-bold mb-4 ">{t('order.title')} Status</h2>
 ...
 {pieData.length > 0 && (
 <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="card p-6">
 <h2 className="text-xl font-bold mb-4 ">{t('order.title')} Bar Chart</h2>
 <ResponsiveContainer width="100%" height={300}>
