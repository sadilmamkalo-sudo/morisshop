...
export default function AdminBlog() {
 ...
 <div className="grid md:grid-cols-3 gap-4 mb-4">
 <div className="md:col-span-3"><label className="block text-sm font-medium mb-1">Description (AR)</label><textarea value={form.description.ar} onChange={e => setForm({ ...form, description: { ...form.description, ar: e.target.value } })} className="input"></textarea></div>
 <div><label className="block text-sm font-medium mb-1">Description (FR)</label><textarea value={form.description.fr} onChange={e => setForm({ ...form, description: { ...form.description, fr: e.target.value } })} className="input"></textarea></div>
 <div><label className="block text-sm font-medium mb-1">Description (EN)</label><textarea value={form.description.en} onChange={e => setForm({ ...form, description: { ...form.description, en: e.target.value } })} className="input"></textarea></div>
 </div>
 <div className="grid md:grid-cols-3 gap-4 mb-4">
 <div className="md:col-span-3"><label className="block text-sm font-medium mb-1">Content (AR)</label><textarea value={form.content.ar} onChange={e => setForm({ ...form, content: { ...form.content, ar: e.target.value } })} className="input min-h-[120px]"></textarea></div>
 <div><label className="block text-sm font-medium mb-1">Content (FR)</label><textarea value={form.content.fr} onChange={e => setForm({ ...form, content: { ...form.content, fr: e.target.value } })} className="input min-h-[120px]"></textarea></div>
 <div><label className="block text-sm font-medium mb-1">Content (EN)</label><textarea value={form.content.en} onChange={e => setForm({ ...form, content: { ...form.content, en: e.target.value } })} className="input min-h-[120px]"></textarea></div>
 </div>
 <div className="grid md:grid-cols-3 gap-4 mb-4">
 <div><label className="block text-sm font-medium mb-1">Slug</label><input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} className="input" placeholder="my-blog-post" /></div>
 ...
 {form.image && <img src={form.image} className="w-32 h-20 object-cover rounded mb-4" />}
 ...
 <td className="px-4 py-3">{p.image ? <img src={p.image} className="w-10 h-10 rounded object-cover" /> : <div className="w-10 h-10 rounded bg-ink-200 dark:bg-ink-700 flex items-center justify-center text-sm"></div>}</td>
 <td className="px-4 py-3 font-medium">{p.title?.en}</td>
 <td className="px-4 py-3 text-sm text-ink-500">{p.slug}</td>
 <td className="px-4 py-3 text-sm text-ink-500">{new Date(p.createdAt).toLocaleDateString()}</td>
 <td className="px-4 py-3"><span className={`px-2 py-1 rounded-lg text-xs font-semibold ${p.published ? 'bg-green-100 text-green-700 dark:bg-green-900/30' : 'bg-ink-50 text-ink-500 dark:bg-gray-800'}`}>{p.published ? 'Published' : 'Draft'}</span></td>
 ...
 {posts.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-ink-500">No posts yet</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
