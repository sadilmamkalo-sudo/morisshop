    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="card-solid overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {backups.map(b => (
              <tr key={b._id} className="border-t border-gray-100 dark:border-dark-700/50 hover:bg-gray-50 dark:hover:bg-dark-800/30">
                <td className="px-4 py-3 font-medium">{b.filename}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{b.size ? `${(b.size / 1024 / 1024).toFixed(2)} MB` : '-'}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{new Date(b.createdAt).toLocaleString()}</td>
                <td className="px-4 py-3">
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => handleDelete(b._id)} className="text-red-500 hover:underline text-sm">🗑️ Delete</button>
                  </div>
                </td>
              </tr>
            ))}
            {backups.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-500">No backups yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
