...
export default function AdminAdmins() {
 ...
 <input type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder={editingId ? 'Leave empty to keep' : t('auth.password')} className="input" required={!editingId} />
 ...
 <label key={perm} className="flex items-center gap-2 p-2 rounded hover:bg-ink-50 dark:hover:bg-ink-800/50 cursor-pointer transition-colors border border-transparent hover:border-ink-200 dark:hover:border-ink-700">
 <input type="checkbox" checked={form.permissions.includes(perm)} onChange={() => togglePermission(perm)} className="text-clay-500 w-4 h-4" />
 ...
 <tbody>
 {admins.map(admin => (
 <tr key={admin._id} className="border-t border-ink-100 dark:border-ink-700/50 hover:bg-ink-50 dark:hover:bg-ink-800/30 transition-colors">
 <td className="px-4 py-3">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded bg-purple-500 flex items-center justify-center text-white text-xs font-bold">{admin.name?.[0]?.toUpperCase()}</div>
 <span className="font-medium">{admin.name}</span>
 </div>
 </td>
 <td className="px-4 py-3 text-sm text-ink-500">{admin.email}</td>
 <td className="px-4 py-3">
 ...
 {admin.permissions?.map(p => <span key={p} className="px-2 py-0.5 bg-ink-50 dark:bg-ink-700 rounded-lg text-xs text-ink-600 dark:text-ink-400">{p.replace('_', '')}</span>)}
 </div>
 </td>
 <td className="px-4 py-3">
 {admin.role !== 'superadmin' && user?.role === 'superadmin' && (
 <div className="flex gap-3">
 <button onClick={() => handleEdit(admin)} className="text-blue-500 hover:underline text-sm font-medium"> {t('common.edit')}</button>
 <button onClick={() => handleDelete(admin._id)} className="text-red-500 hover:underline text-sm font-medium"> {t('common.delete')}</button>
 </div>
 )}
 </td>
 </tr>
