...
export default function AdminGiftCards() {
 ...
 return (
 <div>
 <h1 className="text-3xl font-bold font-display mb-6">Gift Cards</h1>
 ...
 <tbody>
 {cards.map(c => (
 <motion.tr key={c._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="border-t border-ink-200 dark:border-ink-700 hover:bg-ink-50 dark:hover:bg-gray-800/30">
 ...
 <td className="p-4 text-xs text-ink-500">{c.expiresAt ? new Date(c.expiresAt).toLocaleDateString() : '-'}</td>
 </motion.tr>
 ))}
 </tbody>
 </table>
 </div>
 {cards.length === 0 && <p className="text-center text-ink-400 py-8">No gift cards yet</p>}
 </div>
 </div>
 );
}
