      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-6 mb-10">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-moris-500 to-orange-500 flex items-center justify-center text-white text-3xl font-bold shadow-xl shadow-moris-500/30">{user.name?.[0]?.toUpperCase()}</div>
            <div>
              <h1 className="text-3xl font-bold font-display text-gradient">{t('profile.title')}</h1>
              <p className="text-gray-500">{user.email}</p>
            </div>
          </div>

          {loyalty && (
            <div className="card-solid p-6 mb-8 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{loyalty.points} points available</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-moris-500">{loyalty.lifetimePoints}</p>
              ...
              <div className="mt-3 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-moris-500 to-orange-500 rounded-full transition-all" style={{ width: `${Math.min(100, (loyalty.lifetimePoints / 3000) * 100)}%` }} />
              </div>
            </div>
          )}
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="card-solid p-6 md:p-8">
              <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                <span className="w-8 h-8 rounded-lg bg-moris-500/10 flex items-center justify-center text-moris-500 text-sm font-bold">👤</span>
                {t('profile.info')}
              </h2>
              <form onSubmit={handleUpdate} className="space-y-4">
                ...
                <input type="text" value={form.street} onChange={e => setForm({ ...form, street: e.target.value })} placeholder={t('profile.street')} className="input-field" />
                <div className="grid grid-cols-2 gap-4">
                  <input type="text" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} placeholder={t('profile.city')} className="input-field" />
                  <input type="text" value={form.zip} onChange={e => setForm({ ...form, zip: e.target.value })} placeholder={t('profile.zip')} className="input-field" />
                </div>
            ...
            <div className="space-y-8">
              <div className="card-solid p-6 md:p-8">
                ...
                <form onSubmit={handlePassword} className="space-y-4">
                  <input type="password" value={passwordForm.currentPassword} onChange={e => setPasswordForm({ ...passwordForm, currentPassword: e.target.value })} placeholder={t('profile.currentPassword')} className="input-field" required />
                  <input type="password" value={passwordForm.newPassword} onChange={e => setPasswordForm({ ...passwordForm, newPassword: e.target.value })} placeholder={t('profile.newPassword')} className="input-field" required minLength={6} />
                  <button type="submit" className="btn-primary w-full">{t('profile.save')}</button>
                </form>
              </div>
              ...
              <div className="card-solid p-6 md:p-8">
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button key={l.code} onClick={() => setLang(l.code)} className={`flex-1 px-4 py-3 rounded-2xl text-sm font-bold transition-all ${lang === l.code ? 'bg-moris-500 text-white shadow-lg shadow-moris-500/30 scale-105' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                      {l.flag} {l.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="card-solid p-6 md:p-8">
                ...
                {user.subscription?.plan && (
                  <p className="text-xs text-gray-400">Valid until: {new Date(user.subscription.endDate).toLocaleDateString()}</p>
                )}
              </div>

              <button onClick={logout} className="w-full py-3 rounded-2xl bg-red-500/10 hover:bg-red-500/20 text-red-500 font-semibold border-2 border-red-500/20 hover:border-red-500/40 transition-all">
                🚪 {t('auth.logout')}
              </button>
            </div>
          </div>
